	        ...
	        <div>
	         ...
	         <div class="form-option-right" style="float: right;">
	            <div class="proxy-add" title="options">
                 ...
                 <a href="/monitor/{{$cam_id}}/{{$date}}"><button type="button" class="btn btn-model btn-apply">Image</button></a>
	            </div>
	         </div>
	       </div>
	               ...
	               <form action="removeadmin" method="POST">
	                  ...
	                  <table id="example" class="nvr-table">
	                     ...
	                     <tbody class="tbody">
	                        ...
	                        <tr class="color-add" onclick='eventDetail("{!! $video->path !!}")'>
	                           <td>{{$video->occurr_time}}</td>
                             <td>Hanh Lang 405</td>
	                           <td>{{$video->type}}</td>
                             <td>{{$video->cam_id}}</td>
	                        </tr>
	  	...
	  	<div class="row">
            <div class="image-event">
              <div class="frame-preview">
                @if(count($videos) > 0)
                <video width="100%" height="auto" controls playsinline="true" src="{{$videos[0]->path}}" id="video" muted="" autoplay="">
                  Your browser does not support the video tag.
                </video>
                @else
                <video width="100%" height="auto" controls playsinline="true" src="" id="video">
                  Your browser does not support the video tag.
                </video>
                @endif
              </div>
...
<script type="text/javascript">
   function filterDay(e){
     location.replace("/monitor_video/"+$(".cameraids").val()+"/"+e.target.value)
   }

   $(".cameraids").change(function() {
    location.replace("/monitor_video/"+$('.cameraids').val()+"/"+$('#date').val())
  });

   function eventDetail(path){
    $(".location").html("Hanh Lang 405")
    $(".event-type").html(" Video")
